...
\#Todo
       and as section peers, what we want is for all dividing whitespace to be
       a peer\.

       Mostly this is replacing `line-end` with `&line-end`\.


```peg
            doc  ←  (first-section / section) section*
`first-section`  ←  line-end* blocks / line-end+
   ...
   `block-line`  ←  " "* "> " (!"\n" 1)* (!"\n\n" "\n")?
          ...
          table  ←  table-head line-end (table-line line-end)*
   `table-head`  ←  (" "* handle_h* " "*)@table_c
                    "|" (!"\n" 1)* &line-end
   `table-line`  ←  (" "*)@(#table_c) "|" (!line-end 1)* &line-end

           list  ←  (list-line / numlist-line)+
      list-line  ←  margin-l-line / indent-l-line
   numlist-line  ←  margin-n-line / indent-n-line

`margin-l-line`  ←  ("- ")@list_c (!line-end 1)* line-end
                    (!(" "* list-num)
                    (" "+)@(>list_c) !"- "
                    (!line-end 1)* line-end)*
`indent-l-line`  ←  (" "+ "- ")@list_c (!line-end 1)* line-end
                    (!(" "* list-num)
                    (" "+)@(>=list_c) !"- " (!line-end 1)* line-end)*

`margin-n-line`  ←  list-num@numlist_c (!line-end 1)* line-end
                    (!(" "* "- ")
                    (" "+)@(>numlist_c)
                    !list-num (!line-end 1)* line-end)*
`indent-n-line`  ←  (" "+ list-num)@numlist_c (!line-end 1)* line-end
                    (!(" "* "- ")
                    (" "+)@(>=numlist_c)
                    !list-num (!line-end 1)* line-end)*

     `list-num`  ←  [0-9]+ ". " ; candidate: `/ ".. "`
...
`note-line-end`  ←  "\n"+ "   " &note-line
     ...
     link-close  ←  "]: "
         ...
         drawer  ←  drawer-top line-end
                    (!drawer-bottom block line-end*)*
                    drawer-bottom
                    ;; :[contents]:, ::[contents]::
   `drawer-top`  ←  " "* d_lead (!"\n" !d_tail 1)* d_tail &"\n"
`drawer-bottom`  ←  " "* d_match (!"\n" !d_tail 1)* d_tail &"\n"
