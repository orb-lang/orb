...
That said, it's mostly repetition of the same pattern: bookend matching any
number of a sentinel character, at the beginning and end of a string\.
...
`.body` is not the class we want here\. It should call the prose function again
on the freshly constructed table; it should just return instances of `word`
and `punct` but well\. We need testing here and an algorithm which can
accomodate unwrapping the single return value in place\.
